...
export function MatchPage({ data }: { data: AppData }) {
  const { fixtureId } = useParams();
  const m = data.matches.find((x) => x.fixture_id === Number(fixtureId));
      ...
      <div className="max-w-3xl mx-auto py-10 px-4">
        ...
        <div className="card p-8 mt-6 text-center">
          <p className="text-slate-300 light:text-slate-600">
            Pro tento zápas ještě nejsou stažená detailní data (prototyp má zatím jen jeden
            zápas s plnou analýzou).
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      ...
      <footer className="text-xs text-slate-600 text-center py-6">
        Data: SportMonks · vygenerováno {new Date(data.generated_at).toLocaleString("cs-CZ")}
      </footer>
    </div>
  );
}
